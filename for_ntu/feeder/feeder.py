...
sys.path.append("/tmp/pycharm_project_885/AAP_CNN-pytorch-master")
try:
...
class Feeder(torch.utils.data.Dataset):
    ...
    def load_data(self):
        ...
        # load data
        if self.mmap == True:
            self.data1 = np.load(self.data1_path, mmap_mode='r')
            self.data2 = np.load(self.data2_path, mmap_mode='r')
        else:
            self.data1 = np.load(self.data1_path, mmap_mode=None) # directly load all data in memory, it more efficient but memory resource consuming for big file
            self.data2 = np.load(self.data2_path, mmap_mode=None)
        ...
        if self.debug:
            ...
            self.data1 = self.data1[0:100]
            self.data2 = self.data2[0:100]
            self.num_frame_path = self.num_frame_path[0:100]
            self.sample_name = self.sample_name[0:100]

        self.N, self.C, self.T, self.V, self.M = self.data1.shape

    def coordinate_transfer(self):
        data_numpy = self.data1
...
def test(data_path, bone_path, label_path,num_frame_path, vid=None):
    import matplotlib.pyplot as plt
    loader = torch.utils.data.DataLoader(
        dataset=Feeder(data_path, bone_path, label_path, num_frame_path),
        batch_size=64,
        shuffle=False,
        num_workers=2,
    )
    ...
    if vid is not None:
        ...
        data, bone, label = loader.dataset[index]
...
if __name__ == '__main__':
    ...
    data_path = "/home/hhe/hhe_first_file/data_set/NTU-RGB-D-CV/xview/val_data.npy"
    bone_path = "/home/hhe/hhe_first_file/data_set/NTU-RGB-D-CV/xview/val_bone.npy"
    core_path = "/home/hhe/hhe_first_file/data_set/NTU-RGB-D-CV/xview/val_core.npy"
    label_path = "/home/hhe/hhe_first_file/data_set/NTU-RGB-D-CV/xview/val_label.pkl"
    num_frame_path="/home/hhe/hhe_first_file/data_set/NTU-RGB-D-CV/xview/val_num_frame.npy"
    dataset = Feeder(data_path, bone_path, label_path, num_frame_path,
                     random_valid_choose=False,
                     random_shift=False,
                     random_move=False,
    ...
    # test(data_path, bone_path, label_path, num_frame_path)