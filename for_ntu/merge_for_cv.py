...
parser.add_argument('--data_dir', default='/home/hhe/hhe_first_file/data_set/NTU-RGB-D-CV', help='root directory for all the datasets')
parser.add_argument('--datasets', default='/xview', choices={'/kinetics', '/xsub', '/xview'},
                    help='the work folder for storing results')
parser.add_argument('--alpha1', default=1, help='weighted summation for feature1')
parser.add_argument('--alpha2', default=1, help='weighted summation for feature2')
...
train_data_bone = open(root_dir+'/saved_feature/NTU-RGB-D-CV/AIF_CNN' + '/data_bone_train.pkl', 'rb')
train_data_bone = pickle.load(train_data_bone)
val_data_bone = open(root_dir+'/saved_feature/NTU-RGB-D-CV/AIF_CNN' + '/data_bone_val.pkl', 'rb')
...
train_bone_core = open(root_dir+'/saved_feature/NTU-RGB-D-CV/AIF_CNN' + '/bone_core_train.pkl', 'rb')
train_bone_core = pickle.load(train_bone_core)
val_bone_core = open(root_dir+'/saved_feature/NTU-RGB-D-CV/AIF_CNN' + '/bone_core_val.pkl', 'rb')
val_bone_core = softmax(np.array(pickle.load(val_bone_core)))

train_core = open(root_dir+'/saved_feature/NTU-RGB-D-CV/AIF_CNN' + '/data_core_train.pkl', 'rb')
train_core = pickle.load(train_core)
val_data_core = open(root_dir+'/saved_feature/NTU-RGB-D-CV/AIF_CNN' + '/data_core_val.pkl', 'rb')
