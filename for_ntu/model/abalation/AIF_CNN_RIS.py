...
class AIF_CNN(nn.Module):
    ...
    def __init__(self,
        ...
        metric1_numpy = np.eye(25).astype('float32') + 0.01 * np.random.randn(25).astype('float32')
        self.adat_metrices1 = nn.Parameter(torch.from_numpy(metric1_numpy))
        self.relu1 = nn.ReLU()
        ...
        # motion
        metric1_numpy_m = np.eye(25).astype('float32') + 0.01 * np.random.randn(25).astype('float32')
