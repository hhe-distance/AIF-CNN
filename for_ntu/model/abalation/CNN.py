...
class AIF_CNN(nn.Module):
    ...
    def __init__(self,
        ...
        metric3_numpy = np.eye(25, 26).astype('float32')
        self.adat_metrices3 = nn.Parameter(torch.from_numpy(metric3_numpy), requires_grad=False)
        ...
        metric4_numpy = np.eye(13, 14).astype('float32')
        self.adat_metrices4 = nn.Parameter(torch.from_numpy(metric4_numpy),requires_grad=False)
        ...
        metric3_numpy_m = np.eye(25, 26).astype('float32')
        self.adat_metrices3_m = nn.Parameter(torch.from_numpy(metric3_numpy_m), requires_grad=False)
        ...
        metric4_numpy_m = np.eye(13, 14).astype('float32')
        self.adat_metrices4_m = nn.Parameter(torch.from_numpy(metric4_numpy_m), requires_grad=False)
        ...
        metric5_numpy = np.eye(7, 8).astype('float32')
        self.adat_metrices5 = nn.Parameter(torch.from_numpy(metric5_numpy),requires_grad=False)
